...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, caret position, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the available MCPs in the agent environment. Use the available-tools map when present. Otherwise inspect the configured MCP servers (for example a `mcps/` directory) for enabled servers.
...
- `readonly`: `false` (agent mode). **Do not use a read-only mode.** It strips MCP access, which disables MCP-backed investigators entirely. The source control investigator would be safe in read-only, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
- `readonly`: `false` (agent mode). The synthesizer's quality check spot-verifies citations, which can require MCP access. A read-only mode strips MCPs and defeats that.
