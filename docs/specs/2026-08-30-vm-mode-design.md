...
- **Trigger:** manual (`/vm`). No auto-reminder, no always-on mode.
...
- **Modular files.** `SKILL.md` is a concise overview; deeper content lives in
  `playbooks/` (workflows) and `references/` (role detail). This keeps a single
  file from growing too long.
...
1. **Frontmatter** — `name: vm`, a description, and manual trigger (no
   `mode: true`, no reminder).
...
| Situation | Route |
|-----------|-------|
| Implementation, requirements clear | `playbooks/implementation.md` → **brainstorming** |
| Implementation, foggy or too big for one session | `playbooks/implementation.md` → **wayfinder** |
| Investigation / understand a thing | `playbooks/investigation.md` → **research** role |
...
- **watchdog** — takes a PR to green: polls status, resolves conflicts,
  addresses comments, then stops with a final report. Never loops past green.
...
- **research** — builds a picture for an investigation or understanding
  question: context, clues, a formed view, open ambiguities, and a confidence
  read, returned as a cited report.
