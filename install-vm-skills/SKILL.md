...
- `favorites.json`. The manifest. Entries carry `name`, `source`, `scope` (mine/external), and an optional `required` flag for vm.
...
- `required`. The skills the `vm` mode needs to work (a mix of mine and external).
...
python3 scripts/install.py --required   # exactly what vm needs
```

`vm` checks its required skills on start. If any is missing, it routes here with `--required`.
...
- `required`. Optional `true` for skills the `vm` mode depends on.
...
# mark a favorite as required by vm (or not)
